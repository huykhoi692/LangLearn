...
### Chạy app local (nếu muốn chạy thủ công)
```bash
python3 -m http.server 8000
# hoặc
...
### Ghi chú test deterministic
- Test dùng `localStorage` mock state để kiểm soát dữ liệu đầu vào.
- Test mock Supabase client qua script route, không gọi Supabase thật.
