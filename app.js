...
async function upsertDayToSupabase(studyDate, dayData) {
  ...
  if (vocabRows.length) {
    await supa.from('vocab_items').upsert(vocabRows, { onConflict: 'user_id,study_date,word' });
  }
  ...
  if (grammarRows.length) {
    await supa.from('grammar_items').upsert(grammarRows, { onConflict: 'user_id,study_date,point,exercise' });
  }
}
